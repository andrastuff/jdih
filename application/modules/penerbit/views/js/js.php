...
var table 	= $('.dataTables-fetch').DataTable( {
        ...
        buttons: [
            ...
            language: {
            search: '<span>Cari penerbit:</span> _INPUT_',
        ...
        "bInfo" : false,
		"ajax": {
            "url": ServeUrl+"penerbit/api_list",
            "type": "GET",
	...
	$('.dataTables-fetch tbody').on( 'click', '#open', function () {
		...
		window.location.href = ServeUrl+"penerbit/open/"+data['id'];
    } );
	...
	$("#form-data").submit(function(event) {
        ...
        if($.isNumeric(id)){
            var path = ServeUrl+"penerbit/api_update";
        }else{
            var path = ServeUrl+"penerbit/api_save";
        }
	...
	function remove(id){
		Swal.fire({
            ...
            if (result.value) {
				$.ajax({
                    data: {"id" : id},
                    url: ServeUrl+"penerbit/api_delete",
                    method: 'POST',
                    complete: function(response){                
                        if(response.status == 201){
                            Swal.fire
