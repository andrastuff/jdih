...
var table 	= $('.dataTables-fetch').DataTable( {
        ...
        buttons: [
            ...
            language: {
            search: '<span>Cari Status:</span> _INPUT_',
	...
	$('.dataTables-fetch tbody').on( 'click', '#open', function () {
		...
		window.location.href = ServeUrl+"status/open/"+data['id'];
    } );
	...
	$("#form-data").submit(function(event) {
        ...
        if($.isNumeric(id)){
            var path = ServeUrl+"status/api_update";
        }else{
            var path = ServeUrl+"status/api_save";
        }
	...
	function remove(id){
		Swal.fire({
            ...
            if (result.value) {
				$.ajax({
                    data: {"id" : id},
                    url: ServeUrl+"status/api_delete",
                    method: 'POST',
                    complete: function(response){                
                        if(response.status == 201){
                            Swal.fire
