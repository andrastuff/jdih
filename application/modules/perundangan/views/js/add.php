...
$('.btnstep1').hide();
	...
	function getListUrusan(div, def) {
        $.ajax({
            data: "",
            url: ServeUrl+"urusanpemerintahan/api_list_all",
            timeout: 600000,
            type: 'GET',
            dataType: 'json',
            complete: function(response) {
                ...
                if(response.status == 200){
                    var html = '';
                    html += '<option selected disabled>Urusan Pemerintahan ...</option>';
                    for (let i = 0; i < data.length; i++) {
                        html += '<option value="'+data[i].id+'">'+data[i].name+'</option>';
                    }
                    $('#'+div).html(html);
      ...
      $('.subjenis').hide();
        if ($('#typeDokumen').val() != "") {
                ...
                if ($('#typeDokumen').val() == 1) {
					...
					getListUrusan('urusan_pemerintahan');
					}
                    ...
                    complete: function(response){ 		
                        if(response.status == 200){
							...
							if (response.responseJSON.data.tipe_dokumen == 1) {
							 ...
							 getListUrusan('urusan_pemerintahan', response.responseJSON.data.urusan_pemerintahan);
							 ...
							 $("select[name=tahun_terbit1]").val(response.responseJSON.data.tahun_terbit);
							 ...
							 $("select[name=tahun_terbit2]").val(response.responseJSON.data.tahun_terbit);
							 ...
							 $("select[name=tahun_terbit3]").val(response.responseJSON.data.tahun_terbit);
