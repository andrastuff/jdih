...
var table 	= $('.dataTables-fetch').DataTable( {
        ...
        "bInfo" : false,
		"ajax": {
            "url": ServeUrl+"perundangan/api_list",
            "type": "GET",
	...
	$('.dataTables-fetch tbody').on( 'click', '#open', function () {
		...
		window.location.href = ServeUrl+"perundangan/open/"+data['id'];
    } );
	
	$('.dataTables-fetch tbody').on( 'click', '#edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
         window.location.href = ServeUrl + 'perundangan/edit/' + data['id']
	...
	function remove(id){
		Swal.fire({
            ...
            if (result.value) {
				$.ajax({
                    data: {"id" : id},
                    url: ServeUrl+"perundangan/api_delete",
                    method: 'POST',
                    complete: function(response){                
