...
<script>
$('#sideBidanghukum').addClass('active');
$('#openMasterData').addClass('in');
var method = '';
var table 	= $('.dataTables-fetch').DataTable( {
        ...
        buttons: [
            ...
            language: {
            search: '<span>Cari Bidang Hukum:</span> _INPUT_',
        ...
        "bInfo" : false,
		"ajax": {
            "url": ServeUrl+"bidanghukum/api_list",
            "type": "GET",
	...
	$('.dataTables-fetch tbody').on( 'click', '#open', function () {
		...
		window.location.href = ServeUrl+"bidanghukum/open/"+data['id'];
    } );
	...
	$('.dataTables-fetch tbody').on( 'click', '#edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
         ...
         $("input[name=integrasi]").val(data['integrasi']);
	...
	$("#form-data").submit(function(event) {
        ...
        if($.isNumeric(id)){
            var path = ServeUrl+"bidanghukum/api_update";
        }else{
            var path = ServeUrl+"bidanghukum/api_save";
        }
	...
	function remove(id){
		Swal.fire({
            ...
            if (result.value) {
				$.ajax({
                    data: {"id" : id},
                    url: ServeUrl+"bidanghukum/api_delete",
                    method: 'POST',
                    complete: function(response){                
                        if(response.status == 201){
                            Swal.fire
