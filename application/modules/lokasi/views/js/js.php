...
var table 	= $('.dataTables-fetch').DataTable( {
        ...
        buttons: [
            ...
            language: {
            search: '<span>Cari Lokasi:</span> _INPUT_',
	...
	$('.dataTables-fetch tbody').on( 'click', '#open', function () {
		...
		window.location.href = ServeUrl+"lokasi/open/"+data['id'];
    } );
	...
	$("#form-data").submit(function(event) {
        ...
        if($.isNumeric(id)){
            var path = ServeUrl+"lokasi/api_update";
        }else{
            var path = ServeUrl+"lokasi/api_save";
        }
	...
	function remove(id){
		Swal.fire({
            ...
            if (result.value) {
				$.ajax({
                    data: {"id" : id},
                    url: ServeUrl+"lokasi/api_delete",
                    method: 'POST',
                    complete: function(response){                
                        if(response.status == 201){
                            Swal.fire
